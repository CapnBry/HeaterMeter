...
{
    spi_clk_last = read_c0_count();   // assume clock just transitioned
    spi_ps_d1_c0 = ( ( *gpio_output | spi_di_mask ) & ~spi_cl_mask ) & ~spi_cs_mask; // din hi, clock low
    spi_ps_d1_c1 = spi_ps_d1_c0 | spi_cl_mask;  // din hi, clock hi
    spi_ps_d0_c1 = spi_ps_d1_c1 & ~spi_di_mask;  // din low, clock hi
    spi_ps_d0_c0 = spi_ps_d0_c1 & ~spi_cl_mask;  // din low, clock hi
 ...
 *   -Precalculate port states - allows bit changes via assignment (1 cycle)
...
static INLINE void spi_io_v_o ( unsigned char data_out )
{
    ...
    if ( data_out & ( 1 << 7 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 6 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 5 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 4 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 3 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 2 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 1 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
    if ( data_out & ( 1 << 0 ) ) { *out = d1_c0; *out = d1_c1; }
    else { *out = d0_c0; *out = d0_c1; }
}
...
static INLINE unsigned char spi_card_write_multi_o ( unsigned int addr, unsigned char *buf, int blocks, unsigned char first, unsigned char last )
{
    ...
    if ( first ) {
        ...
        if ( r != 0x00 ) {
            LOG_ERR ( "Write_multi command error: %02x, cycles: %d\n", r, i );
            return 2;
        }
    }
...
static INLINE unsigned char spi_card_read_multi_o ( unsigned int addr, unsigned char *buf, int blocks, unsigned char first, unsigned char last )
{
    ...
    if ( first ) {
        ...
        if ( r != 0x00 ) {
            LOG_ERR ( "Read_multi command error: %02x, cycles: %d\n", r, i );
            goto err2;
        }
    }
