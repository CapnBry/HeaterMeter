...
static int init_card_mm ( struct card_info *c )
{
    ...
    if ( ( ( ocr[0] & 0x60 ) >> 5 ) == 2 )
        c->type |= CARD_TYPE_HC; // std/high capacity
...
static int init_card_sd ( struct card_info *c )
{
    ...
    if ( ( r & ~R1_SPI_IDLE ) == 0 ) {
        ...
        if ( ( r & ~ ( R1_SPI_IDLE | R1_SPI_ILLEGAL_COMMAND ) ) > 0 ) {
...
static int init_card ( struct card_info **ret )
{
    ...
    if ( r < 0 )
        goto err;   // SD card init failed

    // Not an SD card so attempt MM card initialization.
    if ( rcode == 1 )
        rcode = init_card_mm ( c );
    if ( r < 0 )
        goto err;   // MM card init failed
